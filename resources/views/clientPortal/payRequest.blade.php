...
@section('content')
 ...
 <div class="content-wrapper">
    ...
    <div class="card-body">
        ...
        <h4 class="card-title">Client Pay Request </h4>
                    ...
                    @foreach($data as $pay)
                        <tr>
                            ...
                            <td class="status-column"">
                                <form action="{{ route('ClientPortal.updateStatus', $pay->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <select name="status" class="form-select" onchange="this.form.submit()">
                                        <option value="Pending" {{ $pay->status == 'Pending' ? 'selected' : '' }}>Pending</option>
                                        <option value="Incomplete" {{ $pay->status == 'Incomplete' ? 'selected' : '' }}>Incomplete</option>
                                        <option value="Complete" {{ $pay->status == 'Complete' ? 'selected' : '' }}>Complete</option>
                                    </select>
                                </form>
                            </td>
